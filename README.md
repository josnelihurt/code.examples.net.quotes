...
Static YAML: `docs/openapi/auth.openapi.yaml`, `docs/openapi/quotes-v0.openapi.yaml`, `docs/openapi/quotes-v1.openapi.yaml`, `docs/openapi/quotes-v2.openapi.yaml`, plus `docs/openapi/quotes-v3.openapi.json` generated from the v3 proto (see [docs/proto-transports.md](docs/proto-transports.md)). Refresh with `./scripts/update-contracts.sh` (Podman/Docker via [`Dockerfile.build`](Dockerfile.build)) after Api/DTO changes — see [docs/api.md](docs/api.md) (stub: [contracts/api-contracts.md](contracts/api-contracts.md)).
...
When the bottom PR of a stack squash-merges, the layer above is retargeted but still carries the bottom layer as a real commit while the base carries the same diff as the squash — GitHub then reports the upper PR **CONFLICTING**, `gh pr update-branch` refuses, and every atomic merge of the layers above fails ([issue #10](https://github.com/josnelihurt/code.examples.net.quotes/issues/10); merge-me holds with this recipe instead of a bare "merge failed"). The repair — the one sanctioned exception to "never force-push mid-stack branches" — replays only a branch's own commits onto the new base, bottom-up:
