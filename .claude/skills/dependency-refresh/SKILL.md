...
3. **Plan (deps-planner, parallel).** Launch two `deps-planner` agents **in parallel, in a single message** — one per surface (NuGet / infra). Each brief is self-contained: the absolute repo root, the audit slice verbatim, the manifest paths to read, and the facts-only rule. Collect the two batch tables.
