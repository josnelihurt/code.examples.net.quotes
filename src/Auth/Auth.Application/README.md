...
| **Errors as values** | Expected failures are returned, not thrown, so every caller sees the same shape and the edge maps it once. | `ErrorOr<LoginResult>`, `AuthErrors.InvalidCredentials`, `AuthErrors.MissingToken` | [error flow](../../../docs/architecture.md#error-flow); `Quotes.Domain.QuoteErrors` |
...
**`AuthErrors` is public API.** The codes travel to clients as the ProblemDetails `errorCode` extension, so `auth.invalid_credentials` and `auth.token_missing` are as much a contract as the URL path. The `ErrorType` chosen for each one is what picks the status code at the edge: `Error.Unauthorized` produces 401 for bad credentials, `Error.Validation` produces 400 for a missing token. Note where the second one is *raised*: the endpoint, not the service, because the token is missing before there is anything for the service to check. `JwtAuthExtensions.TokenMissingErrorCode` in ServiceDefaults reuses the same string for the resource API's 401 challenge, and a test pins the two spellings together.
...
| `Abstractions/AuthErrors.cs` | `public static class` | The context's error catalog; codes are public contract. | `InvalidCredentials` → `Error.Unauthorized("auth.invalid_credentials", "Invalid credentials.")`; `MissingToken` → `Error.Validation("auth.token_missing", "An access token is required.")` |
